...
int32_t  InPlaceParser::Parse(const char *str,InPlaceParserInterface *callback) // returns true if entire file was parsed, false if it aborted for some reason
{
    ...
    mLen = (int32_t)strlen(str);    // Find out the length of the input string
    if ( mLen )
    {
        if ( (mLen+1) > mStringLen)    // If the string is larger than our current buffer
        {
            mStringLen = (mLen + 1) * 2;  // Allocate a buffer twice as large as needed
            free(mStringBuffer);
...
const char ** InPlaceParser::GetArglist(char *line,int32_t &count)
{
	...
	while ( !EOS(*foo) && argc < MAXARGS )
	{
		...
		{
			foo = AddHard(argc,mArgv,foo); // add any hard separators, skip any spaces
